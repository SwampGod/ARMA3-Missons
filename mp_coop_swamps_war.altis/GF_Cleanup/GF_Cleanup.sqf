...
[] spawn {
	while {true} do {
    _time = diag_tickTime + 23;	//	every  50 second
    _i = 0;
    waitUntil {
	sleep 1;
    _i = _i + 1;		
...
_Systemchat_Enabled				= true;	//	Show notifications
_Show_info 						= true;	//	Show notification , counting all the list

_Time_to_next_category			= 1;	// 	The time to move to the next clean category 

_Delete_distance				= 20;	//	Meters distance from Players to delete the items
...
_11_Destructed_Vehicles			= true;
_12_EmptyDetector_MP_triggers 	= true;
_13_Plane_Parts					= true; 
_14_Destruction_Crater			= true; 

//________________ set the time to run / loop _____________

sleep 2400;	//	set the time , in how many second to delete again , if you select to be on continuous (loop)
